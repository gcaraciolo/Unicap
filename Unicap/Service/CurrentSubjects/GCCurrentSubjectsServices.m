...
- (void)getInformationsFromCurrentPeriodWithCompletition:(void (^)(NSArray *))completition
    ...
    [GCBaseService doGETRequestURL:urlAuth params:params completition:^(id response) {
        ...
        completition([self scrapCurrentSubjects:document]);
...
#pragma mark - refatoring everything below

-(NSArray *)scrapCurrentSubjects:(HTMLDocument *)document {
    ...
    return [self fillStudentCurrentSubjects:tableContents];
}

- (NSArray *) fillStudentCurrentSubjects:(NSArray *)tableContents {
    
    NSMutableArray *currentSubjects = [NSMutableArray new];
    
    int i = 0;
    for (NSArray *row in tableContents) {
        
        //TODO: ver como verificar se chegou no fim da tabela. por hora colocar valor estatico
        //linha branca!
        ...
        subject.subjectCode     = [GCUtils contentValueFromHTMLElement:row[0]];
        subject.name            = [GCUtils contentValueFromHTMLElement:row[1]];
        subject.classCode       = [GCUtils contentValueFromHTMLElement:row[2]];
        subject.classroom       = [GCUtils contentValueFromHTMLElement:row[3]];
        subject.schedule        = [GCUtils contentValueFromHTMLElement:row[4]];
        subject.workload        = [GCUtils contentValueFromHTMLElement:row[5]];
        subject.credits         = [GCUtils contentValueFromHTMLElement:row[6]];
        subject.period          = [GCUtils contentValueFromHTMLElement:row[7]];

        [currentSubjects addObject:subject];
        
        i++;
    }
    
    return [currentSubjects copy];
